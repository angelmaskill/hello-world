...
import starter.consumer.QueueConsumer;
import starter.consumer.QueueConsumerFactory;
import starter.event.DataEvent;
import starter.event.DisruptorEventFactory;
import starter.provider.DisruptorProvider;
import starter.thread.DisruptorThreadFactory;
...
public class DisruptorProviderManage<T> {
    ...
    public DisruptorProviderManage(final QueueConsumerFactory<T> consumerFactory,
        ...
        this.executor = new ThreadPoolExecutor(consumerSize, consumerSize, 0, TimeUnit.MILLISECONDS,
                ...
                DisruptorThreadFactory.create("cloud_disruptor_consumer_" + consumerFactory.fixName(), false), new ThreadPoolExecutor.AbortPolicy());
    }
    ...
    public void startup() {
        Disruptor<DataEvent<T>> disruptor = new Disruptor<>(new DisruptorEventFactory<>(),
                size,
                DisruptorThreadFactory.create("disruptor_provider_" + consumerFactory.fixName(), false),
                ProducerType.MULTI,
